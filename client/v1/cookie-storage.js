...
CookieStorage.prototype.getCookieValue = function (name) {
    ...
    return new Promise(function(resolve, reject) {
        self.storage.findCookie(CONSTANTS.TLD, '/', name, function(err, tld_cookie) {
            if (err) return reject(err);
            if (!_.isObject(tld_cookie)) {
                self.storage.findCookie(CONSTANTS.HOSTNAME, '/', name, function(error, h_cookie) {
                  if (error) return reject(error);
                  if (!_.isObject(h_cookie)) return reject(new Exceptions.CookieNotValidError(name));
                  resolve(h_cookie)
                })
            } else {resolve(tld_cookie);}
        })
    });
};
    ...
    return new Promise(function(resolve, reject) {
        self.storage.findCookies(CONSTANTS.TLD, '/', function(err, cookies){
            ...
            if (cookies) {
              ...
              self.storage.findCookies(CONSTANTS.HOSTNAME, '/', function(error, h_cookies){
                if (error) return reject(error);
                resolve(h_cookies || [])
              })
            }
